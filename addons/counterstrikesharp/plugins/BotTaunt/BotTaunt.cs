...
public sealed class BotTauntPlugin : BasePlugin, IPluginConfig<BotTauntConfig>
{
    ...
    private async Task ReplyToPlayerChatAsync(AiChatRequest request, BotTauntConfig config)
    {
        ...
        try
        {
            reply = await FetchAiReplyAsync(request, config);
        }
        catch
        {
        ...
        reply = NormalizeAiReply(reply, config.MaxAiReplyLength) ?? BuildFallbackAiReply(request);
    ...
    private async Task PrintMvpAiTauntAsync(MvpAiTauntRequest request, BotTauntConfig config)
    {
        ...
        try
        {
            taunt = await FetchMvpAiTauntAsync(request, config);
        }
        catch
        {
    ...
    private static async Task<string?> FetchAiReplyAsync(AiChatRequest request, BotTauntConfig config)
    {
        for (var attempt = 0; attempt < 2; attempt++)
        {
            var reply = await FetchAiReplyFromSiteApiAsync(request, config);
            if (!string.IsNullOrWhiteSpace(reply))
            {
                return reply;
            }
    ...
    private static async Task<string?> FetchMvpAiTauntAsync(MvpAiTauntRequest request, BotTauntConfig config)
    {
        for (var attempt = 0; attempt < 2; attempt++)
        {
            var reply = await FetchMvpAiTauntFromSiteApiAsync(request, config);
            if (!string.IsNullOrWhiteSpace(reply))
            {
                return reply;
            }
    ...
    private static async Task<string?> FetchAiReplyFromSiteApiAsync(AiChatRequest request, BotTauntConfig config)
    {
        ...
        using var httpRequest = new HttpRequestMessage(HttpMethod.Post, config.AiApiUrl);
        ...
        var body = BuildAiRequestBody(
            config,
            "你是 Counter-Strike 2 对局里的 BOT 玩家，正在用游戏聊天回复真人玩家。请优先用中文，尽量只说一句短话，通常 10 到 35 个汉字左右。语气可以嘲讽、斗嘴、玩梗或接话，要像游戏内聊天，不要解释规则，不要输出多段说明。",
            $"玩家：{request.PlayerName}（{request.PlayerTeam}，{(request.PlayerAlive ? "存活" : "死亡")}）\nBOT：{request.BotName}（{request.BotTeam}）\n战场情况：{request.BattleSummary}\n玩家刚说：{request.PlayerMessage}\n请用 {request.BotName} 的口吻直接回复一句简短中文聊天。");
        ...
        using var response = await AiHttpClient.SendAsync(httpRequest, timeout.Token);
    ...
    private static async Task<string?> FetchMvpAiTauntFromSiteApiAsync(MvpAiTauntRequest request, BotTauntConfig config)
    {
        if (string.IsNullOrWhiteSpace(config.AiApiUrl))
        {
            return null;
    ...
    private static Dictionary<string, object> BuildAiRequestBody(BotTauntConfig config, string systemPrompt, string userPrompt)
    {
